...
Execucao:
  ...
  (requer variaveis de ambiente configuradas)
"""
...
# Rota para buscar clientes (baseado no script do usuÃ¡rio)
...
# ---------------------------------------------------------------------
# 1. AUTENTICAÃ‡ÃƒO (Reuso da lÃ³gica validada)
# ---------------------------------------------------------------------
async def obter_token_autenticado() -> str:
    ...
    async with httpx.AsyncClient(
        ...
        if not m:
            ...
            raise RuntimeError("Token SolarZ nÃ£o encontrado.")
...
async def fetch_clientes_page(client: httpx.AsyncClient, page: int) -> Dict[str, Any]:
    ...
    logger.info(f"Buscando pÃ¡gina {page} via POST...")
    
    # Tentativa com POST e payload de paginaÃ§Ã£o
    payload = {
        "page": page,
...
async def fetch_all_clientes() -> List[Dict[str, Any]]:
    ...
    headers = {
        ...
        "Origin": "https://pages.solarz.com.br", # Manter compatibilidade com headers originais se necessÃ¡rio
    ...
    async with httpx.AsyncClient(
        ...
        while True:
            ...
            # Tenta extrair conteÃºdo de diferentes estruturas comuns
            ...
            if total_pages is not None:
                ...
                # Se nÃ£o tem totalPages, para se vier menos que o tamanho da pÃ¡gina
                if len(content) < SOLARZ_PAGE_SIZE:
                    break
                page += 1
                
        logger.info(f"Total de clientes extraÃ­dos: {len(all_items)}")
        return all_items
...
async def run_sync_logic():
    logger.info("=== SincronizaÃ§Ã£o de Clientes SolarZ ===")
